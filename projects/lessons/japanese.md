...
Not starting from zero — reads kana, knows basic copula sentences and common kanji. The work is
...
- Diagnostic: advanced beginner (~high N5). Reads hiragana, knows greetings + X は Y です, reads
  common kanji (日本語/私/食べる). Gaps: verb conjugation, particles, output on IME.
- Covered: を object particle (type `wo`, not お — he fixed this mid-session), 〜たい "want to do"
  = stem + たい, and IME typing (パ vs バ handakuten; `cha`→ちゃ; space to convert お茶).
- Cleared: 食べたい correct; を correct by end; typed お茶 with conversion perfectly.
- Shaky: u-verb stem hop — kept writing のむたい (dict form) instead of のみたい (飲む→飲み, mu→mi).
- Next: ru-verb vs u-verb groups + the stem hop. That's the root of the たい error.
