...
export default async function AdminTenantDetailPage({ params }: PageProps) {
  ...
  let tenantDetail: {
    ...
    ownerEmail: string
    memberCount: number
    discordConnections: { id: string; bot_username: string | null; guild_id: string; status: string; last_heartbeat: string | null; error_message: string | null }[]
    apiKeys: { id: string; api_key_type: string; key_hint: string; status: string; validated_at: string | null }[]
    serviceConnections: { id: string; service_name: string; status: string; created_at: string; metadata: Record<string, string> | null }[]
    subscription: { plan: string; stripe_status: string | null; stripe_subscription_id: string | null; current_period_start: string | null; current_period_end: string | null; cancel_at_period_end: boolean; trial_end: string | null; created_at: string } | null
    auditLog: { id: string; action: string; admin_user_id: string; adminEmail: string; created_at: string; metadata: Record<string, unknown> | null }[]
  }
  ...
  try {
    ...
    // Admin credentials not available — render with placeholder data
    tenantDetail = {
      id,
      name: 'Tenant',
      plan: 'free',
  ...
  return (
    <AdminLayout pageTitle={`${tenantDetail.name} — Tenant Detail`}>
