...
This prints `ALL GATES PASSED (11/11)` on success. It is the exact command CI executes, so a green
...
| G13. Assertion discipline | `node scripts/check-assertion-discipline.mjs` | No frontend test asserts nothing, and no test whose only matcher is `toBeDefined` or `toBeTruthy` exists outside the shrink-only `assertion-baseline.json` ledger. A static source scan, so it is independent of the frontend suite. See [`GATES.md`](./GATES.md) section 11. |
...
| G11. Engine observability | `node scripts/check-observability.mjs` | The engine still emits warnings, the legitime and free-portion split, and a structured boundary error. All ten spec-defined manual review flag codes are declared and exercised, and neither hardcoded line that suppressed observability has returned. See [`GATES.md`](./GATES.md) section 9. |
