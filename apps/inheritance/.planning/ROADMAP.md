...
| 18. One Fact Set, Keyed on Date of Death | 0/6 | Planned | - | FACT-01..04 all open. Planned 2026-08-01: six plans, 18-01 through 18-06, across five waves. **Planning measured, rather than quoted, the audit claim.** Confirmed: only `decedent_name` crosses the engine boundary (`$caseId.tax.tsx:56`); the tax path holds a second editable date at `DecedentTab.tsx:92-93` writing `EstateTaxWizardState.decedent.dateOfDeath` while the succession path writes `EngineInput.decedent.date_of_death`; there is no equality check anywhere; and `prePopulateFromEngineInput`, an adapter that would have carried the date, has **zero** production callers. Measured with figures: one fact set computed at `2017-12-31` vs `2018-01-01` moves `deductionRules` PRE_TRAIN -> TRAIN, the medical deduction 40,000,000 -> 0 centavos and `tax_due` 100,500,000 -> 30,000,000 centavos. **One audit sub-claim is FALSE and it reshapes the phase:** RA 11642 retroactivity does not read the date of death — `engine/src/flags.rs:143` keys on the adoption *decree date*, every `date_of_death` occurrence in `engine/src/` is a test fixture, and a corpus double-compute over all 173 committed inputs (171 computable) changed **0** outputs. So roadmap success criterion 5 as literally worded cannot be met without answering `LAWYER-08`, and the phase's own cross-cutting constraint forbids moving a legal outcome; the phase routes the shared date into the engine and reports the invariance as a finding. New BLOCKING gate **G34** at order 11 — after G2 which builds the WASM artifact it reads, before G3 where the suite halts — to be observed red on three injected regressions before registration. `bash scripts/ci-gates.sh` exit 0 is NOT achievable and is explicitly not claimed; journey step `tax-tab-0` will fail on its reference image and is withheld for human review, not approved |
