...
| 17. Citation Integrity — One Attribution Authority | 0/6 | Planned | - | CITE-01..05 planned 2026-08-01 in six plans across five waves. **Measured at planning, not estimated**: across the 171 computable committed corpus inputs (652 heir rows), **615 rows (94.3%) carry a narrative article absent from that row's `legal_basis`** — 564 of them `Art. 887` against table rows reading `Art. 996`, `Art. 980` or `Art. 999` — and **0 of the 24 distinct `legal_basis` strings the engine emits resolve** against `NCC_ARTICLE_DESCRIPTIONS`, so the citation pill is dead for every article on every case, and the PDF prints `Art. 996: Art. 996`. Four strings stay unresolved even after `parseArticleKey`: `Art. 892 ¶1`, `Art. 892 ¶2` (paragraph suffix rejected by the regex) and `Art. 983`, `Art. 999` (no map entry). `predictScenario`/`computeMock` measured at **zero importers**, so `CITE-04` is a clean deletion. **A G28 trap was found and defused at planning**: naively removing the eight prose citations makes `Art. 176` and `Art. 179` `REGISTERED ARTICLE ABSENT` and drifts `Art. 887`/`Art. 972`; preserving each literal as a traceability comment was simulated and gives `producedBy deltas: 0`, so `engine/legal-rules.json` needs no edit and is excluded from every plan's `files_modified`. **Known not deliverable by this phase**: `bash scripts/ci-gates.sh` exit 0 — still owner-blocked at G3 by Phase 16's two decisions — and the journey steps `results-view`/`results-family-tree`, already withheld for human review, which this phase changes again and does not approve |
