...
- [x] **Phase 22: Deed of Extrajudicial Settlement — Schedule of Shares** - The schedule clause as pasteable text and DOCX, nothing else of the deed
...
**Why phases 24–27 have no row in the table below.** Gate G33
(`node scripts/check-planning-truth.mjs`) derives every row from `.planning/phases/<NN>-<slug>/`, and
raises `ROADMAP PLAN COUNT` for a row whose phase directory does not exist. None of the four has
...
19 were planned and executed on 2026-08-01 and have rows; Phase 20 was planned on 2026-08-01 and has a
row reading `0/7` and `Planned`; Phases 21 and 22 were planned and executed on 2026-08-01 and read
`8/8` and `Complete`; Phase 23 was planned on 2026-08-02 and has a row reading `0/8` and `Planned`.
...
| 23. The Instrument — Letterhead, Attribution, Warnings in the PDF | 0/8 | Planned | - | INST-01..05 planned 2026-08-02 across 8 plans in 5 waves. **THE AUDIT'S CENTRAL LINE IS CONFIRMED AT THE LINE**: `ActionsBar.tsx` calls `downloadPDF(input, output, null)` and `EstatePDF` gates the whole letterhead on that third argument, so the letterhead `/settings` captures and stores can never render — 23-05 loads the signed-in user's profile through a new `loadCurrentFirmProfile()` and passes it, with all three absence paths (no supabase config, no session, no row) resolving `null` rather than rejecting. **ONE FIELD THE ATTRIBUTION BLOCK NEEDS DOES NOT EXIST**: `user_profiles` stores an IBP number but no Roll of Attorneys number, so 23-01 adds migration `016` with one nullable column, carries it through both row mappers and the settings form, and renames nothing. **WARNINGS BECOME ONE MODEL WITH TWO RENDERERS**, the same architecture Phases 21 and 22 used: `warnings-lines.ts` builds every line once and the PDF section and the on-screen panel render that array, so the two surfaces cannot disagree about which warnings exist — 23-06 asserts field-by-field parity between them. **THE MARKDOWN AND CITATION DEFECTS WERE RE-MEASURED, AND THE AUDIT'S STRING IS STALE WHILE THE DEFECT IS NOT**: asterisks are confined to narratives, and the repeated-article line is now emitted by the Phase-17 citation resolver, so 23-03 fixes the live shape rather than the audit's literal. **NO LEGAL QUESTION IS TOUCHED**: every line this phase prints is either a field the lawyer typed into `/settings` or engine text quoted verbatim; `lawyer-decisions.json` and `LAWYER-AGENDA.md` are untouched and no reading is adopted. New BLOCKING gate **G39** `instrument parity` at order 36 — after the halt at G17 (order 15), the placement rule inherited from Phases 21 and 22 — extracting the exported PDF's text and asserting the letterhead, all five attribution lines, every engine warning, zero markdown asterisks and no repeated article, to be observed red on four injections plus one pre-change rendering before registration; a run examining zero warnings, zero attribution lines or zero citations exits 1 with `INSTRUMENT CORPUS EMPTY`. Gate set **37 → 38**, `order` the only field touched on any existing gate. **INST-05 IS THE ONE HUMAN-JUDGEMENT STEP AND IT IS FENCED**: 23-08 makes `journey/pdf-approve.mjs` refuse without `--by`, keeps `maxDiffPixels` at 0, requires every page image to be opened and its differences written down, requires G39 green first, and requires the executor to STOP and report BLOCKED if any page shows a change outside the five this phase made. **`bash scripts/ci-gates.sh` exit 0 is NOT claimed**: the suite halts at G17, and G20/G21 remain owner-blocked. |
