...
**Why phases 21–27 have no row in the table below.** Gate G33
(`node scripts/check-planning-truth.mjs`) derives every row from `.planning/phases/<NN>-<slug>/`, and
raises `ROADMAP PLAN COUNT` for a row whose phase directory does not exist. None of the seven has
...
19 were planned and executed on 2026-08-01 and have rows; Phase 20 was planned on 2026-08-01 and
therefore has a row, reading `0/7` and `Planned`.
