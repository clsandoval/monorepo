...
**Plans**: 6 plans, 5 waves (planned 2026-08-01 in `.planning/phases/19-wizard-persistence-that-actually-persists/`)
...
| 19. Wizard Persistence That Actually Persists | 0/6 | Planned | - | SAVE-01..05 **planned 2026-08-01**, not executed. Six plans across five waves in `.planning/phases/19-wizard-persistence-that-actually-persists/`. Planning **measured** the audit claim rather than quoting it: `setAutoSaveInput` has exactly **2** call sites and both are inside the `useEffect(..., [caseId])` load effect at `$caseId.tsx:41-77`, `WizardContainerProps` declares only `onSubmit` and `defaultValues`, and the hook's seven unit cases all **pass** against a hook the application cannot reach. Two corrections to the audit's framing, both measured: (1) the reference guard is not inert — because `prevInputRef` initialises to the first render's `null`, opening a case performs **one** save 1.5s after load whose payload is byte-identical to what was just read, so the product writes when it has nothing to write and not when it does; (2) `frontend/test-baseline.json`'s `min_total_tests: 2119` floor is **10 tests** above the measured `2109`, and `frontend/scripts/check-test-baseline.mjs:186` compares with a strict less-than — so the floor can be cleared by **adding** passing tests, without any baseline being edited. That outcome is measured in 19-06 and is deliberately **not** claimed in advance. SAVE-05 is proven by a **standalone gate script** (`frontend/journey/persistence.mjs`, new blocking gate **G35** at order 17, after G19) and not by a registry step, because a registry step needs a first reference image and producing one is a human visual judgement no plan may make; the gate creates and deletes its **own** case row, since `RESETS['case-alpha-no-output']` does not restore `input_json` and typing into the seeded Alpha case would redefine what G19 money parity computes against. The save indicator renders **nothing at idle**, matching `EstateTaxWizard.tsx:103`, so the five approved wizard reference images stay valid. **EXPLICITLY NOT CLAIMED**: `bash scripts/ci-gates.sh` exit 0 — **G20/G21** remain registered blocking gates whose scripts `4ccf06270` deleted, and retiring a gate is owner action under CLAUDE.md invariant 2 enforced by G5 |
