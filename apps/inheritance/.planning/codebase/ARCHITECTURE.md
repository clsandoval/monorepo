...
- `engine/src/wasm.rs`: `#[wasm_bindgen] pub fn compute_json(input: &str) -> Result<String, JsValue>` — parses `EngineInput` via `serde_json::from_str`, runs `run_pipeline()`, serializes `EngineOutput` via `serde_json::to_string`. Parse/serialize errors are emitted as a JSON payload carrying a `kind` field, which `frontend/src/wasm/bridge.ts` turns into a typed value through its exported `EngineError` type and `parseEngineError` helper.
...
| Wizard current step index | Seeded from the URL, then held in local state | **URL-addressable.** `readInitialWizardState()` in `frontend/src/components/wizard/WizardContainer.tsx` reads the succession wizard's starting step from the URL on first paint, and the estate-tax wizard's tab index is addressable the same way — a journey gate can deep-link a step instead of clicking Next | `frontend/src/components/wizard/WizardContainer.tsx`, `frontend/src/components/tax/EstateTaxWizard.tsx` |
...
| Onboarding step | Local `useState<OnboardingStep>` (`'firm' | 'profile' | 'done'`) | Not addressable from the URL; seed by controlling whether an org already exists for the signed-in user (no org → step 'firm' shown; org exists → auto-redirect away from `/onboarding` entirely) | `frontend/src/routes/onboarding.tsx` |
