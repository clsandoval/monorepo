...
**Claim:** `src/wasm/pkg/this_string_appears_nowhere_in_the_tree.js`
...
## D4 — the Art. 900 ¶2 three-month window

**Claim:** `KNOWN DIVERGENCE: engine/src/step5_legitimes.rs`
**Where:** `specs/inheritance-engine-spec.md` — search for that literal marker.
**Contradicted by:** `is_articulo_mortis` never differences `date_of_marriage` against
`date_of_death`, so the spec's three-month trigger is stated but not computed. Phase 14 recorded this
under the explicit divergence marker rather than silently closing it. Correcting the engine here
would require reading Art. 900 ¶2, which is a point of Philippine law and therefore not an agent's
decision.
**Owning requirement:** No requirement owns this.
