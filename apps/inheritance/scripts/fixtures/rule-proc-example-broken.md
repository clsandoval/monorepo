...
If the article already appears in `engine/legal-traceability.lock`'s `untraced_articles` array,
...
`engine/legal-traceability.lock`, and gate G28 green.
...
**Vector function:** test_function_that_does_not_exist
...
5. **Register.** `engine/legal-rules.json` holds
   ...
   and `Art. 172` is absent from `engine/legal-traceability.lock`, because it now has a vector.
