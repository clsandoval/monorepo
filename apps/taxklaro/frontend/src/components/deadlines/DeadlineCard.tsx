...
export function DeadlineCard({ milestoneKey, dueDate, description, completed, computationTitle, computationTitles }: DeadlineCardProps) {
  ...
  return (
    <div
      className={`bg-gray-50 rounded-lg transition-colors duration-200 overflow-hidden flex ${getUrgencyClasses(dueDate, !!completed)} ${completed ? 'opacity-60' : ''}`}
    >
      {/* Date column */}
      <div className="flex flex-col items-center justify-center px-4 py-4 min-w-[4rem] bg-muted/40 text-center">
        <span className="text-2xl leading-none text-foreground">{day}</span>
