...
| SMTP Host | Provided by email service (Postmark, SendGrid, Resend, etc.) |
| SMTP Port | 587 (STARTTLS) or 465 (SSL) |
| SMTP User | Provided by email service |
| SMTP Password | Provided by email service |

**Required DNS records for email authentication:**

| Type | Name | Value | Purpose |
|------|------|-------|---------|
| TXT | `@` | `v=spf1 include:<smtp-provider-spf> ~all` | SPF — authorizes SMTP provider to send on behalf of `daimon.ai` |
| TXT | `<selector>._domainkey` | `v=DKIM1; k=rsa; p=<public-key>` | DKIM — cryptographic email signature (generated by SMTP provider) |
| TXT | `_dmarc` | `v=DMARC1; p=quarantine; rua=mailto:dmarc@daimon.ai` | DMARC — policy for handling SPF/DKIM failures |

These DNS records are not needed for launch (Supabase default SMTP works), but should be set up before sending significant email volume to avoid deliverability issues.
