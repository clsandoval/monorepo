...
| [frontend/docs-pages.md](frontend/docs-pages.md) | Docs layout (260px sidebar, DocsTopbar, 780px article, full typography + callout box system); Quick Start guide (8 numbered steps: account creation → Discord bot setup → Anthropic API key → add key to Daimon → connect Discord → verify live → first message → optional integrations); Troubleshooting section (Bot Not Connecting, API Key Issues, Tool access, Billing); What's Next; Discord & Core Tools tool reference (Discord 7, Dub 2, Credentials 1, GitHub 1); Toggl tool reference (34 tools in 6 categories: Time Entry/Project/Task/Workspace Member/Project User/Workspace Reports with Admin Only badge system, 34-row quick-reference table, example outputs per tool); responsive (3 breakpoints, mobile hamburger slide-over); accessibility (12 ARIA specs, focus trap). Other doc pages (FAQ, billing) added in aspects 4.8g–4.8h. | Partial (4.8a–4.8c done) |
