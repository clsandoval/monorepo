...
| [frontend/docs-pages.md](frontend/docs-pages.md) | Docs layout (260px sidebar, DocsTopbar, 780px article, full typography + callout box system); Quick Start guide (8 numbered steps: account creation → Discord bot setup → Anthropic API key → add key to Daimon → connect Discord → verify live → first message → optional integrations); Troubleshooting section (Bot Not Connecting, API Key Issues, Tool access, Billing); What's Next; Discord & Core Tools tool reference (Discord 7, Dub 2, Credentials 1, GitHub 1); Toggl tool reference (34 tools in 6 categories); LinkedIn & Google Analytics tool reference (LinkedIn 17 + Google Analytics 4); Fly, ACP, Decision Hub, Onyx & Bluedot tool reference (Fly 9 + ACP 4 + Decision Hub 4 + Onyx 2 + Bluedot 4 = 23 tools; each with full params, example XML output, platform auth notes, error scenarios, usage patterns; footer nav → Linear Tools; 23-row quick-reference table; accessibility table); Other doc pages (FAQ, billing, Linear) added in aspects 4.8f–4.8h. | Partial (4.8a–4.8e done) |
