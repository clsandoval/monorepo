...
| [frontend/docs-pages.md](frontend/docs-pages.md) | Docs layout (260px sidebar, DocsTopbar, 780px article, full typography + callout box system); Quick Start guide (8 numbered steps: account creation → Discord bot setup → Anthropic API key → add key to Daimon → connect Discord → verify live → first message → optional integrations); Troubleshooting section; What's Next; Discord & Core Tools tool reference (Discord 7, Dub 2, Credentials 1, GitHub 1); Toggl tool reference (34 tools in 6 categories); LinkedIn & Google Analytics tool reference (LinkedIn 17 + Google Analytics 4); Fly, ACP, Decision Hub, Onyx & Bluedot tool reference (23 tools); Linear tool reference (6 remote MCP tools: linear_list_issues, linear_search_issues, linear_get_issue, linear_create_issue, linear_update_issue, linear_list_teams — each with full params, example output, error cases, usage examples; Remote MCP architecture note; "Setting Up Linear" walkthrough; 6-row quick-reference table); Complete Tool Index (master index of all 90 tools across all 5 platforms with anchor links to doc pages); FAQ and billing doc pages pending (4.8g–4.8h). | Partial (4.8a–4.8f done) |
