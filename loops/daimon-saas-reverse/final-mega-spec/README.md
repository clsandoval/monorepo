...
| [frontend/docs-pages.md](frontend/docs-pages.md) | Docs layout (260px sidebar, DocsTopbar, 780px article, full typography + callout box system); Quick Start guide (8 numbered steps: account creation → Discord bot setup → Anthropic API key → add key to Daimon → connect Discord → verify live → first message → optional integrations); Troubleshooting section (Bot Not Connecting, API Key Issues, Tool access, Billing); What's Next; Discord & Core Tools tool reference (Discord 7, Dub 2, Credentials 1, GitHub 1); Toggl tool reference (34 tools in 6 categories); LinkedIn & Google Analytics tool reference (LinkedIn 17 tools: posts/ads-campaigns/ad-analytics-library/conversions/events/leads/org-stats; Google Analytics 4 tools: ga_run_report/ga_get_traffic_overview/ga_get_top_pages/ga_get_campaign_performance; each with params, example output, empty/error state, accessibility specs); responsive (3 breakpoints, mobile hamburger slide-over); accessibility (ARIA specs, focus trap). Other doc pages (FAQ, billing) added in aspects 4.8g–4.8h. | Partial (4.8a–4.8d done) |
