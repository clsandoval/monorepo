...
- [ ] **Stage 1**: Install shadcn/ui CLI, run `npx shadcn@latest init`, configure `components.json` with `src/components/ui` path alias, set up Daimon CSS variables in `globals.css`, add `cn()` utility to `src/lib/utils.ts`
