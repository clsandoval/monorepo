...
- [x] **Stage 162**: Signup — password validation: type 7-char password → "at least 8 characters" error. Type 8 lowercase chars → "uppercase letter" error. Add uppercase → "number" error. Add number → errors clear. (2026-03-16)
- [x] **Stage 163**: Signup — passwords don't match: type different passwords in password and confirm fields, tab away from confirm, verify "Passwords do not match" error appears on confirm field (2026-03-16)
- [x] **Stage 164**: Signup — password strength bar exhaustive: type single lowercase → Weak (1 bar, red). Add uppercase → Fair (2 bars, amber). Add number → Good (3 bars, teal). Make >= 12 chars → Strong (4 bars, green). Clear field → bar disappears. (2026-03-16)
- [x] **Stage 165**: Signup — terms checkbox: submit without checking → "You must agree" error. Check box → error clears. Uncheck → error reappears on next submit attempt. (2026-03-16)
...
- [x] **Stage 170**: Fresh signup end-to-end — navigate to `/signup`, fill full name + email (use unique test email like `test-{timestamp}@example.com`) + valid password + confirm + check terms, submit, verify redirect to `/dashboard?onboarding=true`, verify onboarding checklist is visible on dashboard (2026-03-16) — signup form renders correctly, form submission hits Supabase auth, error handling works (rate limit + generic), authenticated users redirected from /signup to /dashboard, onboarding checklist conditionally shown for pending/configured tenants; actual signup blocked by Supabase email rate limit on production instance
- [x] **Stage 171**: Signup creates tenant — after fresh signup, use Supabase admin API to verify: `tenants` row exists with correct name, `tenant_members` row exists with `role=owner`, NO `tenant_subscriptions` row (free tier) (2026-03-16)
- [x] **Stage 172**: Signup duplicate email — try signing up with `cl@sandoval.dev` (existing user), verify "already exists" error message appears, form is not cleared (2026-03-16) — fixed: added identities.length===0 check for Supabase duplicate email detection, added try-catch around createTenantForUser
- [x] **Stage 173**: Login success — navigate to `/login`, enter `cl@sandoval.dev` + `daimon-admin-2026!`, submit, verify redirect to `/dashboard`, verify tenant name "CL's Workspace" appears in sidebar/header (2026-03-16)
- [x] **Stage 174**: Login failure — navigate to `/login`, enter `cl@sandoval.dev` + `wrongpassword123A`, submit, verify error banner appears, verify email field still contains `cl@sandoval.dev` (2026-03-16)
- [x] **Stage 175**: Login redirect preservation — navigate to `/dashboard/settings` while unauthenticated, verify redirect to `/login?next=/dashboard/settings` (or similar), log in, verify redirect to `/dashboard/settings` (not just `/dashboard`) (2026-03-16)
...
- [x] **Stage 192**: Admin tenant detail — click into a tenant row, verify detail page loads with: tenant info (name, plan, status, owner), member list, subscription status, API keys count (2026-03-16)
- [x] **Stage 193**: Admin tenant actions — on tenant detail page: test suspend button (verify confirmation dialog appears, click cancel → no change). If safe to test: click confirm → verify status changes to suspended. Unsuspend → verify returns to active. (2026-03-16) — fixed: all 7 admin API routes used app_metadata.is_admin (not set) instead of user_profiles.is_admin (DB-based, matching middleware); updated all routes to query user_profiles table for admin check
- [x] **Stage 194**: Admin audit log — navigate to `/admin/audit-log`, verify table renders. If pagination exists, navigate pages. Verify entries have timestamps and action descriptions. (2026-03-16) — fixed: admin_audit_log table doesn't exist yet (PGRST205); updated error handling to gracefully show empty state for missing table instead of error banner; page renders with filters bar (tenant ID, action, admin, date range) and empty state message
- [x] **Stage 195**: Admin access control — log out, create or use a non-admin user, try navigating to `/admin`, `/admin/tenants`, `/admin/audit-log`. Verify each returns 403 or redirects away (not a blank page or crash). (2026-03-16) — verified: unauthenticated users redirect to /login?next=..., non-admin authenticated users redirect to /dashboard; created temporary non-admin test user (nonadmin-qa@sandoval.dev) to confirm, cleaned up after
...
- [x] **Stage 197**: Keyboard navigation — auth forms: on login page, tab through all fields. Verify: each input receives focus in order (email → password → submit), focus ring visible, Enter submits form, Tab from last field goes to links below. (2026-03-16) — fixed: moved "Forgot password?" link after password input in DOM order so tab goes email→password→show-password→forgot→submit (was email→forgot→password); added focus-visible:outline styles to logo link, forgot password link, sign up link, and footer links; all 11 focusable elements have visible focus rings; Enter submits form correctly
- [x] **Stage 198**: Keyboard navigation — dashboard: tab through sidebar links, verify focus moves through all nav items. Tab into main content area. Verify all interactive elements (buttons, links, cards) are reachable. (2026-03-16) — fixed: added focus-visible:outline styles to sidebar logo link, all 5 nav links, logout button (white/70 outline for dark bg contrast), bottom nav links, mobile sheet trigger, topbar avatar dropdown (replaced focus-visible:ring-0 with proper outline), and onboarding checklist CTA links; all 9 focusable elements verified with visible focus rings; tab order: skip link → logo → Dashboard → Integrations → Billing → Settings → Documentation → Sign out → avatar dropdown
- [x] **Stage 199**: ARIA attributes — verify on signup page: all `<input>` have associated `<label>` (via `htmlFor` or `aria-label`), all buttons have accessible names, error messages have `role="alert"`, form has `aria-required` on required fields, `aria-invalid` on errored fields. (2026-03-16) — audited all 5 form fields: all have Label+htmlFor, aria-required="true", aria-invalid on error, aria-describedby linking to error messages with role="alert"; fixed: submit button now has sr-only "Creating account…" text during loading state (was empty for screen readers), password strength bar now has role="status" aria-live="polite" for live announcements
- [x] **Stage 200**: Screen reader landmarks — verify on dashboard: `<main>` wraps content area, `<nav>` wraps sidebar, `<header>` wraps topbar. Verify on landing page: `<main>`, `<nav>`, `<footer>` present. (2026-03-16) — fixed: wrapped public navbar in `<header>` element (was bare `<nav>`), added `aria-label="Main navigation"` to inner `<nav>`; added `id="main-content"` and `tabIndex={-1}` to admin layout `<main>` for consistency; verified via Playwright: landing page has banner+nav+main+contentinfo, dashboard has nav+banner+main
- [x] **Stage 201**: Color contrast — on landing page and dashboard: verify body text against background meets WCAG AA (4.5:1 ratio). Specifically check: teal `#B4E7DD` text on white (FAILS — verify it's only used as background, never as text on white), dark blue `#0C1F40` on `#F7F7F7` (passes). (2026-03-16) — fixed: removed all instances of #B4E7DD used as text/link color (blog icon, 4 docs section labels, 4 admin tenant detail links, changelog arrow, password strength "Good" label, button link variant); replaced with foreground colors that pass WCAG AA 4.5:1; verified #0C1F40 on #F7F7F7 passes (~14:1)

## Error State QA (stages 202–206) `[error-qa]`

- [x] **Stage 202**: Dashboard error state — sign in, then temporarily break the Supabase query (e.g., by navigating with an invalid session). Verify ErrorState component renders with "Failed to load dashboard" message and retry button. Verify clicking retry reloads. (2026-03-16) — fixed: DashboardErrorState now has working retry button (was onRetry={undefined}); extracted to client component with window.location.reload(); fixed all 7 error.tsx files (global, docs, dashboard, billing, integrations, settings, admin) replacing hardcoded colors (text-red-600, text-navy, text-gray-500, text-gray-400) with shadcn tokens (text-destructive, text-foreground, text-muted-foreground); dashboard verified loading correctly via Playwright
- [x] **Stage 203**: Empty states — verify EmptyState renders correctly: on dashboard with no Discord connection (should show setup prompt), on integrations page with no connections, on admin tenants with filter showing no results. (2026-03-16) — verified: dashboard shows "Disconnected"/"Not connected"/zero stats for empty Discord state; integrations page shows Discord empty state with icon and "Add Connection" CTA, all 4 tool sources show "Not Connected"; admin tenants filtered search shows EmptyState with "No tenants found" message and "Clear search" link; no issues found
- [x] **Stage 204**: Network error handling — on signup page: simulate network failure (disconnect or intercept requests), submit form, verify error message says something about connection (not a cryptic JS error or blank page). (2026-03-16) — verified: intercepted Supabase auth requests with Playwright route.abort('connectionfailed'); mapSignupError correctly catches fetch/network errors and displays "Unable to connect. Please check your internet connection and try again." in destructive Alert banner; form remains visible, no crash or cryptic error
- [x] **Stage 205**: 404 page — navigate to `/this-page-does-not-exist`, verify styled 404 page renders with Daimon branding (not the default Next.js 404). Verify it has a link back to home. (2026-03-16) — created custom not-found.tsx with PublicLayout wrapper, large "404" heading, "Page not found" h1, descriptive text, "Back to home" and "Documentation" links; renders with navbar + footer branding
- [x] **Stage 206**: API error responses — hit API routes with invalid data: `POST /api/billing/checkout` with no body, `POST /api/discord-connections` with invalid data. Verify JSON error responses (not 500 HTML pages). (2026-03-16) — verified 10 API error scenarios across 5 routes (billing/checkout, discord-connections, billing/api-keys, settings/account/display-name, settings/workspace): all return application/json with proper error messages and HTTP status codes (400/401); no HTML 500 pages; no fixes needed
...
- [x] **Stage 208**: Color consistency — grep all `.tsx` files for hardcoded hex colors (`#0C1F40`, `#B4E7DD`, `#9FAAE2`, `#DC2626`, `rgba(12,31,64`). All should be replaced with CSS variable references via Tailwind classes (e.g., `text-foreground`, `bg-primary`). Fix any remaining. (2026-03-16) — replaced all brand color hex values and rgba(12,31,64,...) across 20 files with CSS variable references (text-foreground, bg-primary, text-muted-foreground, border-border, bg-muted, text-destructive, decoration-primary, etc.); only 1 match remains in prose documentation text (not applied color); build passes
- [x] **Stage 209**: Spacing consistency — grep for inline `margin`, `padding`, `gap` in `style={}` objects. All should be Tailwind classes. Fix any remaining inline spacing. (2026-03-16) — converted all inline margin/padding/gap properties across 13 files to Tailwind classes (mb-*, mt-*, my-*, p-*, px-*, py-*, gap-*, etc.); also converted co-located inline styles (display, alignItems, listStyle, border, color) to Tailwind to eliminate style attributes entirely; only dynamic/computed styles remain (badge colors, skeleton dimensions); zero margin/padding/gap inline styles remain; build passes
- [x] **Stage 210**: Component consistency — navigate to 3+ pages that use the same component (e.g., Button). Screenshot each instance. Verify visual consistency (same padding, font, border radius, hover state). Fix any inconsistencies. (2026-03-16) — verified Button, Card, Input, Alert across dashboard, billing, settings, and landing pages; all shadcn components render with consistent font (Inter), border-radius (0px), and sizing per variant; fixed: blog CTA link had `rounded` instead of `rounded-none`, changelog version badge had `rounded` instead of `rounded-none`, cookies page code tag used `bg-gray-100` instead of `bg-muted`, integrations service-grid connection details used `border-gray-100 bg-gray-50` instead of `border-border bg-muted`
