# Frontier — Filipino Youth Slang Dictionary

## Statistics
- Total aspects: 25
- Analyzed: 9
- Pending: 16
- Convergence counter: 0/3
- Raw terms gathered: 141

---

## Wave 1a: Lyrics Sources

- [x] O Side Mafia — full discography, focus on tracks 2023-2026
- [x] Ex Battalion — recent releases and features 2023-2026
- [x] Skusta Clee — solo tracks and collaborations 2023-2026
- [x] Flow G — solo and featured verses 2023-2026
- [x] Hev Abi — discography 2023-2026
- [x] Matthaios — tracks and features 2023-2026
- [x] Because — discography 2023-2026
- [x] ALLMO$T — tracks 2023-2026
- [x] Shanti Dope — recent releases 2023-2026
- [ ] Tus Brothers — discography 2023-2026
- [ ] JMara / Locos Only / other emerging PH rap acts 2023-2026

## Wave 1b: Social & Community Sources

- [ ] TikTok PH — trending slang compilations, comment sections, Filipino TikTok creators explaining slang
- [ ] Twitter/X Filipino — slang threads, viral tweets using new terms, stan/fan community language
- [ ] Reddit r/Philippines and r/CasualPH — slang discussion threads, "what does X mean" posts
- [ ] YouTube PH — slang explainer videos, reaction videos, vlogs with heavy slang usage
- [ ] Urban Dictionary — Filipino/Tagalog entries added 2023-2026
- [ ] Beki/gay lingo evolution — new terms from LGBTQ+ Filipino community 2023-2026
- [ ] Gaming PH — Discord, Facebook gaming groups, ML/Valorant/Genshin PH community slang
- [ ] Filipino meme pages — Facebook meme groups, IG meme accounts, emerging meme slang

## Wave 2: Define & Validate

- [ ] Define batch 1 — first 50 raw terms gathered from Wave 1
- [ ] Define batch 2 — next 50 raw terms
- [ ] Define batch 3 — remaining raw terms + any new discoveries
- [ ] Cross-reference pass — terms found in multiple sources, verify consistency

## Wave 3: Thesaurus & Taxonomy

- [ ] Synonym/antonym linking — connect related entries across the dictionary
- [ ] Category discovery — cluster terms by organic usage domains

## Wave 4: Synthesis

- [ ] Generate dictionary.md from final dictionary.json
