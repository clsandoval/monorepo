...
Generate images via Nano Banana 2 by running the bundled script. Requires `GEMINI_API_KEY` environment variable.

```bash
python3 .claude/skills/gemini-image-gen/scripts/generate.py "your prompt here" -o output.png
```
...
1. Craft a detailed prompt — be specific about style, composition, lighting, and subject
2. Run: `python3 .claude/skills/gemini-image-gen/scripts/generate.py "<prompt>" -o <output-path>`
